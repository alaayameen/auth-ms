...
public class LoginByMobileController {
    ...
    public AuthenticateRespond loginByMobile(LoginByMobile loginByMobileRequest, String acceptLanguage){
        log.debug("loginByMobile called with LoginByMobile{}", loginByMobileRequest);
        String mobileNumber = loginByMobileRequest.getMobileNumber().replace("+", "");
        Auth authData = authDao.getAuthDataByMobileNumber(mobileNumber);
        
        log.debug("loginByMobile authData {}", authData);
        if(authData != null) {
            if (passwordEncoder.matches(loginByMobileRequest.getPassword(), authData.getPassword())) {
                ...
                log.debug("loginByMobile userInfo {}", userInfoForAdmin);
                ...
                if (userInfoForAdmin != null) {
                    ...
                    if (Boolean.TRUE.equals(userInfoForAdmin.isMarkedForDelete())) {
                        if (Boolean.TRUE.equals(loginByMobileRequest.isRestoreAccount())) {
                ...
                authenticateRespond.setAccessToken(generateTokenFromUserInfo(loginByMobileRequest,mobileNumber,authData.getUserId(),acceptLanguage,authData.getRule(), userInfoForAdmin));
                authenticateRespond.setRefreshToken(refreshTokenService.createRefreshToken(authData.getId()).getToken());
                
                log.debug("loginByMobile token are generated {}", authenticateRespond);
    ...
    private String generateTokenFromUserInfo(LoginByMobile loginByMobileRequest, String mobileNumber, String userId, String acceptLanguage, RuleEnum ruleEnum, UserInfoForAdmin userInfoForAdmin) {
        UserInfo userInfo;
